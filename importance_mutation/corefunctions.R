...
mutate_trait <- function(trait = NULL, mutation_rate = NULL, 
                          ...
                          TraitIsOptima = NULL, BinaryMutate = FALSE,DrawFromRange=TRUE,step_size=step_size) {
...
lapply_wrapper_CPP <- function(XY,
                               ...
                               lineage_track, Test_HWE,metabolic_cost
                               ) {
  if(lineage_track){
 ...
 #   print("Issue here")
    host_fitnessvector<- calculate_host_fitness_cpp(
      ...
      selection_parameter_hosts, env_cond_val_used[per_host_bac_gens],metabolic_cost
    )
    ...
    host_fitnessvector <- host_fitnessvector
      ...
      if (optima_mutate == TRUE) {
        if (mutation_rate_optima > 0) {
          host_microbe_optima <- mutate_trait(trait= host_microbe_optima,mutation_rate =  mutation_rate_optima,mutation_range_low =  -2.5,mutation_range_high = 2.5,TraitIsOptima=FALSE,BinaryMutate = F,DrawFromRange=TRUE,step_size = 0.15)
        }
      }
      if (importances_mutation == TRUE) {
        if (mutation_rate_importance > 0) {
          microbiome_importances <- mutate_trait(trait = microbiome_importances, mutation_rate = mutation_rate_importance,mutation_range_low = 0,mutation_range_high = 0.5,TraitIsOptima=FALSE,BinaryMutate = F,DrawFromRange=FALSE,step_size = 0.1)
        }
      }
